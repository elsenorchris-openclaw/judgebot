#!/usr/bin/env python3
"""Per-station HIGH BUY_NO window sweep (data-driven, Chris). For each station,
sweep candidate windows (open in {3,2.5,2,1.5}, close in {1.5,1.0}) with the
FAITHFUL gated buy-at-open sim, pick the max full-PnL window, flag confidence via
the early/late split (ROBUST=both halves +, SOFT=one half -, THIN=n<15). Close
kept >=1.0 (trading into peak is the robustly losing zone). Stations whose best
window is still negative fall back to the broad (2.0,-1.0). Emits a ready
PUSH_HIGH_TEMP_WINDOW_BY_STATION dict in (before, after) form."""
...
CANDS = [(3.0, 1.5), (3.0, 1.0), (2.5, 1.5), (2.5, 1.0), (2.0, 1.5), (2.0, 1.0), (1.5, 1.0)]
FALLBACK = (2.0, -1.0)
...
print("per-station HIGH BUY_NO window sweep (faithful gated buy-at-open). split @ %s\n" % MED)
chosen = {}
for iata in sorted(STORE):
    ...
    if best is None:
        print("  %-5s  (no window n>=10) -> broad fallback (2.0,-1.0)" % iata)
        chosen[iata] = (FALLBACK[0], FALLBACK[1], "THIN/fallback"); continue
    pnl, oh, ch, n, wr, epnl, lpnl, ne, nl = best
    flag = "ROBUST" if (epnl > 0 and lpnl > 0 and n >= 15) else ("THIN(n<15)" if n < 15 else "SOFT(1half-)")
    use_oh, use_aft, note = (oh, -ch, flag)
    if pnl <= 0:
        use_oh, use_aft, note = (FALLBACK[0], FALLBACK[1], "neg-best->broad fallback")
    chosen[iata] = (use_oh, use_aft, note)
    print("  %-5s best [pk-%.1f,pk-%.1f] full %2.0f%% n=%-3d %+5.1fc | early %+5.1f(%d) late %+5.1f(%d)  %s%s" % (
        iata, oh, ch, wr, n, pnl, epnl, ne, lpnl, nl, flag,
        "  -> SHIP broad (neg)" if pnl <= 0 else ""))

print("\nPUSH_HIGH_TEMP_WINDOW_BY_STATION = {")
for iata in sorted(chosen):
    oh, aft, note = chosen[iata]
    print('    "K%s": (%.1f, %.1f),  # %s' % (iata, oh, aft, note))
print("}")
