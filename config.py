...
GUARDRAILS = {
    ...
    "max_bet_low_series_usd": 5.0,  # 2026-06-06 (Chris): 1->5. Raise LOW — edge backtests +7.22c/ct fwd-chained (14mo, +ve every month) / +11.7c posted-at-mid; live ~20 fills over 3 days all POSITIVE @85% fill, matching the backtest (probe +$3.72, Jun5 +$4.84). $3 was already validated; this goes past it to $5. KDEN LOW-NO $10 carve-out restored (PUSH_LOW_NO_BET_BY_STATION). Rollback ->1. # 2026-06-02 (Chris): 10->1. START-SMALL whole-bot $1 cap (LOW min-buy is already PUSH_MIN_BUY_USD_LOW=0.40 so $1 cap is fine). Raise once live LOW fills validate. # (prior 2026-05-26: bumped 1->3 harvest) after live validation of the PUSH_LOW_POST_AT_MID probe. n=11 fills over 2 days at $1: +$3.72 realized (+$0.34/trade), 85% fill rate, +~11c/contract -- matches the +11.7c posted-at-mid backtest mechanism. Per-trade absolute risk small; LOW BUY_YES (previously the cross-loser) flipped to +$4.31/7 in the probe sample as expected (cheaper entry post-at-mid changes EV sign). KDEN BUY_NO B-bracket stays at $10 via PUSH_LOW_NO_BET_BY_STATION; all other LOW (incl. BUY_YES any station, T tails) lifts from $1 to $3.
...
PUSH_LOW_NO_BET_BY_STATION = {"KDEN": 10.0}  # 2026-06-06 (Chris): restored the KDEN LOW-NO $10 carve-out atop the $5 base — KDEN LOW-NO is the one robustly-both-halves cell (+32.7c / +25.6c). KNYC still held out (early-half negative). # 2026-06-02 (Chris): cleared the KDEN $10 carve-out for the whole-bot $1 START-SMALL cap (uniform $1 LOW). Re-add {"KDEN": ...} when caps go back up. # (prior 2026-05-24: KDEN 10.0; KNYC HELD) -- independent faithful backtest shows NYC LOW-NO early-half NEGATIVE (-7.5c; net +8c but not robustly both-halves on my split). DEN robust both halves (+32.7/+25.6c). Re-add "KNYC": 10.0 after reconciling methodology (NO-only? RMSz).
