...
AUTO_EXEC_LOW_ENABLED: bool = False  # 2026-06-17 (autonomous): DISABLED -- LOW is structurally -EV at EVERY gate, large-sample + both-halves. Large reconstruction (374 LOW-NO candidates x market results, ground-truth-anchored): LOW-NO all -10.3c/ct, B-NO -6.0, B-NO+pno>=.55 -5.1, and the CURRENT shipped LOW config (B-NO/pno.55/50-80c/sp5/edge8) = -2.4c/ct H2 -5.7 (n=18) -- ALL both-halves NEGATIVE; no price/spread/edge stack makes it robustly + (the few green cells are tiny-n H1-only noise). Confirms lifetime live (-16c/ct n=19, every config era neg). This is the evidence-backed disable flagged in memory ("next bleed -> disable, not a 6th tweak") after 6+ LOW config eras. HIGH (both-halves +) carries the strategy; removing a -EV book improves P&L (running losers for "volume" is wrong -- the 6/17 HIGH edge-bar 18->14 adds PROFITABLE volume instead). The +7.22c/ct "fwd-chained" claim below was the original reconstruction that live REFUTED. Rollback ->True (against the wallet). cf project_blend_faithful_backtest_20260616.  # (prior 2026-06-02 RE-ENABLE rationale, now refuted; 2026-05-27 OFF)
