...
AUTO_EXEC_LOW_ENABLED: bool = True  # 2026-06-17 LATE (autonomous, ERROR CORRECTION): RE-ENABLED at $5. The same-day disable (text retained below) was a METHODOLOGY ERROR -- that 374-candidate analysis did NOT restrict to the trading window, so it averaged in OUT-OF-WINDOW LOW candidates (h_to_min 3-5h, which lose -6..-10c/ct) that the bot NEVER trades, dragging the average to -2.4c/ct. Restricted to the actual window [min-3,-1.5] (= all the bot does), distinct-ticker reconstruction = +32c/ct n=8 100%WR (H2+31), AND the faithful REAL fills agree: the 5 current-gated LOW fills (6/13-16) settled +$3.08 / 80%WR (the wallet, the clincher). The LOW window-curve also confirms [1.5,3.0] is right (deeper LOW leads LOSE -- opposite of HIGH, where deeper wins). The -16c/ct lifetime was the PRE-GATE loose era. ⚠️small n (5 real / 8 recon), thin book (~1-3 fills/night) -> judge on settled fills; revert ->False only if the IN-WINDOW gated fills bleed live. cf project_blend_faithful_backtest_20260616.
