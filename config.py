...
PUSH_EDGE_TIER_SIZING_ENABLED: bool = True  # 2026-06-07 (Chris): edge-tier HIGH sizing. Flat sizing (_compute_size spends to the cap regardless of edge) leaves ~11% on the table — higher-edge bets have higher EV/contract, so concentrate size on them. Applied in nn_shadow_strategy.pure_nn_decide (HIGH only; LOW stays $1 B-NO flat). Rollback ->False.
