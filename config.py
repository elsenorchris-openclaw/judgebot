...
PUSH_HIGH_MAX_BET_DEFAULT: float = 3.0  # 2026-06-09 (Claude, Chris-approved): MIDDLE-PATH 1->3 -- re-expand HIGH onto the validated NO-only >=10pp core (see PUSH_HIGH_NO_ONLY + PUSH_MIN_EDGE_PP 6->10). Audit (frozen prod model, 427 summer recon days): NO-only >=10pp +9.4c/ct/66%WR, both halves +, LOSO all 7 folds +, robust to a ~37% blowup-day rate; live ex-6/4 +11.4c/ct. The prior -$879/WR0.41 was UNREPRODUCIBLE with the prod model at any gating level (likely matcher mu). Intermediate ($3, not $8) on a contested-thesis season. Rollback ->1.0. # 2026-06-08 (Chris): SUMMER SWAP 8->1. HIGH (afternoon max) is -EV in summer at ALL edge bars (convective, unpredictable; summer 2025 WR 0.41, -$879; high_edge.py seasonal). The blend's edge is winter/spring; LOW (min) is the summer edge -> de-risk HIGH to $1, size LOW to $3. min-buy GUARDRAILS=0.40 so $1 cap trades. Restore ->8 in fall (~Sep). # 2026-06-06 (Chris): 5->8 (HIGH NO/default only; PUSH_HIGH_YES_MAX_BET_USD stays $5). Now protected by the one-bracket-per-station cap (PUSH_ONE_BRACKET_PER_STATION_HIGH) which cuts the correlated per-forecast tail ~3x (Jun4 counterfactual -$71->-$23), so $8+cap has a SMALLER tail than the old $10-no-cap. Validated edge +16-18c/ct. Backstop max_bet_high_series_usd=$20. Rollback ->5.  # 2026-06-05 (Chris): 10->5 DE-RISK (6/4 HIGH -$58 MTM, edge unproven live -> halve the swing while settled days accumulate; rollback ->10). Prior 6/3 5->10 (HIGH NO/default). Blend HIGH's first real day decoded ~+$21 with the confident/longshot bets winning (OKC NO 29c->+$12.69); window+timing fixes live. Doubling exposure on the validated +16-18c/ct HIGH edge. Backstop guardrail max_bet_high_series_usd=$20 still caps. LOW stays $1. Rollback ->5.  # (prior: 1->5 6/2; 10->1 START-SMALL 6/2)
