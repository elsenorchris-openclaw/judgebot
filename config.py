...
AUTO_EXEC_LOW_ENABLED: bool = False  # 2026-06-18 (autonomous): RE-DISABLED. The 6/17 re-enable (text below) was an over-correction on an n=8 in-window slice. The COMPREHENSIVE EV map (2576 settled blend-eval tickers, all decisions incl SKIPs, distinct-ticker) is decisive: LOW BUY_NO is -EV at EVERY edge band -- neg-edge -5.4c/ct (n770), [0,8) -2.5, [8,15) -12.7, [15,30) -4.0, [30+) -13.6 -- there is NO LOW edge band that's +EV. The in-window +32c/ct (n=8) that justified the re-enable was noise (the full surface dwarfs it), and today (6/18) BOTH live LOW positions are losing (HOU -4.74 / SFO -4.20) -- the +$3.08/n5 is now ~breakeven-to-neg with today. LOW has no edge anywhere; running it bleeds. The ORIGINAL 6/17 disable was right; my re-enable was wrong. Rollback ->True only with a window-restricted AND large-sample +EV demonstration (which the comprehensive map says doesn't exist). cf project_blend_faithful_backtest_20260616.
# --- RETAINED (the FLAWED 6/17 disable rationale, kept as a correction tombstone) ---
# "DISABLED -- LOW structurally -EV: 374-candidate recon LOW-NO -10.3c/ct, B-NO -6.0, current cfg -2.4c/ct n=18..."
# ^ WRONG: not window-restricted; counted out-of-window phantom trades. The in-window gated LOW is +EV (above).
...
PUSH_MIN_EDGE_PP: int = 14  # 2026-06-17 (autonomous, large-sample reconstruction): 18->14 to ADD VOLUME ("barely buying"). The 6/10 "sub-18 bled -$51/n51" finding (below) PRE-DATED the clearance>=1 + tpno>=0.6 gates (shipped 6/11) -- those gates remove exactly the bad sub-18 shapes (thin-clearance B-NO, coinflip T-NO) that caused that bleed. Re-tested WITH the full quality gates on the 982-trade reconstruction (1056 blend-NO candidates x 15266 market results, ground-truth-anchored via tools/replay_backtest.py): HIGH q-gated edge>=14 = +17.7c/ct, BOTH HALVES + (H1+28.3/H2+10.0, n=38) vs edge>=18 +16.2 (n=31); the incremental [14,18) band WITH gates = +24.3c/ct n=7 (flips the pre-gate -9.4). Faithful 24-fill set sign-AGREES ([14,18) real +7.3c/ct n=4). Mechanism: the QUALITY GATES carry the edge; the bar above ~14 was redundant. Adds ~25% HIGH volume; composes w/ the new per-day cap (which bounds the extra correlated exposure). ⚠️LOOSENING = the historically-risky direction + reconstruction is decision-ask-priced (optimistic, but spread<=5 caps slip <=2.5c, can't flip +17.7); faithful confirm is only n=4. REVERT TRIGGER: if the [14,18) band bleeds on LIVE settled fills, back to 18. Judge via replay_backtest.py. # PRIOR 2026-06-10 (Chris: "deep dive tactics, raise edge req etc"): 10->18 -- LIVE-FILL tactic sweep, all 192 blend-era fills incl 6/9 book-resolved. Within the spread<=5/price<=80-clean pool the sub-18pp band is the bleed (-$51.11 n=51 -9.4c/ct) while >=18pp is +$50.75 n=32 +11.5c/ct WR62, POSITIVE IN ALL FOUR SPLITS (H1 +18.5 / H2 +3.4 / odd +5.8 / even +15.0 c/ct -- the only tactic stack that passes). 18 is also the PRE-6/2 pre-registered value ("Rollback ->18" below): this is the live-fill verdict on the 6/2 loosening experiment, not a freshly mined carve. Ships w/ PUSH_MAX_SPREAD_C_HIGH 25->5 + PUSH_MAX_ENTRY_C 90->80 (stack validated together). ⚠️judge on settled fills; expected vol ~4 HIGH fills/day. Rollback ->10. # PRIOR 2026-06-09 (Claude, Chris-approved): SUMMER 6->10 HIGH NO bar -- the $3 size-up rides only the robust >=10pp core. Recon NO 6pp +9.0 vs 10pp +9.4c/ct (+EV & lower var, n 211->149); LIVE 6-10pp NO band was the weak/-EV tier (-18.5c/ct; ex-6/4 still -$25). Rollback ->6 in fall. # 2026-06-07 (Chris): 2->6 HIGH NO bar (variance cut). Edge-bar sweep (high_edgebar.py, 19mo, tail-gate applied, daily-$ aggregated): vs the 2pp bar, raising to ~6pp keeps ~same total $ (+6455 vs +6323) but cuts daily-P/L SD -9% (39.4 vs 43.2) and the WORST day -21% (-48 vs -61); 4pp strictly dominates 2pp (more $ AND less var -> [2,4)pp band is dead weight). Smoother P/L / fewer big forecast-miss days (the June-6 type) at ~no $ cost; more surgical than shrinking size (keeps high-conviction bets). Both halves +. LOW uses its own 8pp bar. Rollback ->2. # 2026-06-02 (Chris): 18->2 AGGRESSIVE volume. The 18 floor was MATCHER-era (matcher's 12-18pp band was -5.7c/bet on real fills). Under the BLEND every edge band down to ~2pp is +EV (blend mu is accurate enough that small disagreements still pay): HIGH NO edge-band sim, net fee — [1,2)pp +5.7c/ct, [2,4) +3, [4,8) +7, [8,12) +9, cumulative >=2pp = 19.7/mo +10.4c/ct vs >=18 only 6.6/mo; total $ ~2x, volume ~3x (~2 HIGH buys/day). Only [0,1)pp is -EV (the ~1.5c fee floor). LOW <=1c also holds at >=2pp (+24.7c/ct n28). Structural protections UNCHANGED (price floors 50/10/30, spread <=15c HIGH/<=1c LOW, thin-margin-NO, NBM veto, sigma band, tail-bet gate stays 25, deep-window). SLIPPAGE is the live risk (low-edge trades are fragile; sim has none) -> validating at $1 + Discord feed. Rollback ->18. cf project_blend_edge_FOUND 6/2 edge-band analysis.
...
PUSH_LOW_MIN_PNO: float = 0.55  # 2026-06-10 (Chris LOW deep-dive): NEW gate -- LOW BUY_NO requires the model itself to lean NO: P(NO) >= this, independent of price. A cheap ask MANUFACTURES edge while the model is ~coinflip (DAL 6/10: P(NO)=0.52 @28c = "23.6pp edge", lost -- the recurring LOW loss shape). Stream sweep: P(NO)>=0.55 flips the kept book +1.4 -> +9.4c/ct (n=10); combined LOW stack (this + 40c floor + spr<=5) = 5/5 winners +43.3c/ct over 8 days incl the bot's 2 real MIA wins. Applied in _try_auto_execute (LOW B-NO only). 0 = off.
...
PUSH_MIN_ENTRY_C_LOW: int = 50       # 2026-06-13 (Chris): 40->50. The 6/13 LOW losers exposed it -- all 3 NOs were bought at market-implied P(NO) < 0.55 (asks 54/46/40c), i.e. the bot bet NO on brackets the MARKET leaned YES (HOU 46c/LAX 40c = market >=54% the bracket HITS). That's the RULE#2 contested-NO shape (market right when it disagrees w/ us) AND the band the stream already flagged weaker: [50,80]c +11.1c/ct WR75 (n8) > [40,80]c +8.4c/ct (n12). A 50c floor = only buy LOW NO when the market is at least NEUTRAL (implied P(NO)>=~0.50), not actively betting against us; blocks the two deadest 6/13 fills (HOU 46/LAX 40), keeps ATL 54 (which also lost -- floor doesn't save a wrong forecast, only removes the most-contested bets). Honest caveat: this re-approaches the 5/28 matcher-era 50c floor that HURT (-$16.75->-$20.48 n=57) -- but that was matcher-mu; under the blend + P(NO)>=0.55 conviction gate the stream prefers 50. LOW remains the un-proven book (0-for-3 under the new stack, never live-positive any era) -- if 50c+conviction still bleeds, the call is AUTO_EXEC_LOW_ENABLED=False, not a 6th tweak. Rollback ->40. # PRIOR 2026-06-10 (Chris): 10->40. Decision-stream sweep (4452 blend-era B-NO rows / 105 tickers / settled results for ALL LOW markets, taker-priced): the LOW bleed concentrates in CHEAP CONTESTED NOs -- sub-40c book = market >=60% sure the bracket hits, and it keeps being right (today's DAL @28c gap23.6 = the shape; price[40,80] +8.4c/ct n12 vs [10,80] +1.4 n15; both today's losers blocked). ⚠️Counter-finding on record: 5/28 matcher-era 50c floor HURT LOW (-$16.75->-$20.48 n=57) -- that was matcher-mu at the old windows; blend-era stream + live fills say the cheap band is where LOW dies (era LOW fills <40c: DAL -4.66, LV -5.32, PHX -4.89, LAX -5.29...). Part of the 6/10 LOW stack (P(NO)>=0.55, spread<=5). Rollback ->10. # PRIOR 2026-05-28 (Chris): BUY_NO floor for the LOW book ONLY. The 50c PUSH_MIN_ENTRY_C is a HIGH-book finding (real fills); applied GLOBALLY it inverted on LOW — LOW NO went -$16.75 -> -$20.48 under the 50c floor (n=57). Restore LOW to its pre-e5d6e01 10c floor (HIGH-only floor). LOW remains net-negative overall = separate strategic question, not solved here. Decision-stream sweep (4452 blend-era B-NO rows / 105 tickers / settled results for ALL LOW markets, taker-priced): the LOW bleed concentrates in CHEAP CONTESTED NOs -- sub-40c book = market >=60% sure the bracket hits, and it keeps being right (today's DAL @28c gap23.6 = the shape; price[40,80] +8.4c/ct n12 vs [10,80] +1.4 n15; both today's losers blocked). ⚠️Counter-finding on record: 5/28 matcher-era 50c floor HURT LOW (-$16.75->-$20.48 n=57) -- that was matcher-mu at the old windows; blend-era stream + live fills say the cheap band is where LOW dies (era LOW fills <40c: DAL -4.66, LV -5.32, PHX -4.89, LAX -5.29...). Part of the 6/10 LOW stack (P(NO)>=0.55, spread<=5). Rollback ->10. # PRIOR 2026-05-28 (Chris): BUY_NO floor for the LOW book ONLY. The 50c PUSH_MIN_ENTRY_C is a HIGH-book finding (real fills); applied GLOBALLY it inverted on LOW — LOW NO went -$16.75 -> -$20.48 under the 50c floor (n=57). Restore LOW to its pre-e5d6e01 10c floor (HIGH-only floor). LOW remains net-negative overall = separate strategic question, not solved here.
PUSH_MAX_ENTRY_C: int = 80  # 2026-06-10 (Chris tactic sweep): 90->80 -- the 6/2 experiment's OWN revert trigger fired: live 80-90c HIGH NO fills = -$22.32 (n=19, the "one 90c loss = 9 wins" risk realized). Mirrors locklag's independently-validated ceil90 lesson one tier down (expensive favorites' last nickels don't cover blowup risk at our obs fidelity). Part of the 6/10 stack (edge>=18, spread<=5). NOTE: PUSH_MIN_ENTRY_C stays 25 -- live <40c NO fills were POSITIVE (+$38.19 n=18), do NOT restore the 50c floor. Rollback ->90. # PRIOR 2026-06-02 (Chris): 80->90 — $1 LIVE EXPERIMENT. Tests buying favorites (80-90c). Favorite-longshot bias is a real inefficiency (favorites underpriced +2.7/+4.4pp); sim says 80-95c NO is +EV under the blend. Risk = variance (one 90c loss = 9 wins) but bounded at $1. WATCH the feed; revert ->80 if 80-90c fills net-negative. Applies to NO+YES+LOW.
...
PUSH_HIGH_NO_MAX_WIND_MPH: float = 7.0     # 2026-06-30 (Chris-approved, wind deep-dive): HIGH-NO STABLE-AIRMASS gate -- skip a HIGH BUY_NO when decision-time SUSTAINED wind > this. The mirror of the LOW frontal gate (PUSH_LOW_FRONT_WIND_MPH) on the HIGH side, but far tighter than the 40mph catch-all above (which never fires -- max observed wind is 26). Elevated late-morning sustained wind = unsettled/frontal/convectively-mixed airmass = the blend forecast (and the "blows-past" NO it implies) is unreliable + temps run suppressed/volatile -> the "correlated hot-miss" tail that clearance/edge could NEVER gate (the signal is meteorological, not in the forecast margin). Real-fill evidence (255 HIGH-NO fills x Kalshi settlements, 5/19-6/30): ~100% of the bot's lifetime -$399 came from fills with sustained wind > 8mph; wind<=7mph = +$153 / 64%WR (6-7mph band +$121 / +2.16/fill) while EVERY band >=8mph nets -$2.2..-2.75/fill. WITHIN-STATION 14/16 (not a station selector; KMIA coastal + KLAS desert are the physical exceptions), era-robust (loose+tight config halves both +), LODO worst-drop +$119, WR gap z=2.43. Mechanism: calm = stable high-pressure = predictable diurnal heating = the blows-past NO is safe. Sustained wind ONLY (gusts are convective -> tier1 PUSH_MAX_WIND_MPH handles those). Cuts ~half of HIGH-NO volume; judge the settled calm-day tape before any size-up. 0 = off (rollback). cf project_blend_wind_gate_20260630.
PUSH_MAX_SPREAD_C_HIGH: float = 5.0       # 2026-06-10 (Chris tactic sweep): 25->5 -- the 6/2 experiment's OWN revert trigger fired: live wide-spread HIGH NO fills bled (spread>=5c = -$46.60 n=21 WR48; [5,10)c alone -18.3c/ct n=20; <5c = +$6.04 n=94). Mechanism: a wide book = thin/uncertain market -- the mid is noise, the "edge" vs it is fictitious, and crossing pays ~half the spread. Tighter than the pre-6/2 15c because the live bleed extends down to 5c ([5,15) is where it concentrates); 15 = the conservative pre-registered fallback if 5 starves volume. Part of the 6/10 stack (edge>=18, ceil 80). Rollback ->15 (pre-6/2) or ->25. # PRIOR 2026-06-02 (Chris): 15->25 — $1 LIVE EXPERIMENT. Crossing a wide spread is partly a mechanical cost (you pay ~half the spread), but the blend edge may clear it; sim says 15-40c spread is +EV under the blend. Testing at $1. WATCH the feed; revert ->15 if wide-spread fills bleed. (Prior 2026-05-23: 15c, matcher backtest -21..-31c/bet >15c.) LOW stays its own gate (PUSH_MAX_SPREAD_C_LOW). 0 = off.
PUSH_MAX_SPREAD_C_LOW: float = 5.0        # 2026-06-10 (Chris LOW deep-dive): 25->5. Stream sweep: spr<=5 = +17.2c/ct (n=8) vs <=25 = +1.4 (n=15); a wide overnight book = the market isn't there, the mid is noise and the maker fill on it is adverse selection. Matches the HIGH spread finding (6/10) and the original 6/2 LOW sim (1c-only +EV). Maker still posts at mid within the <=5c book; taker-fallback unaffected. Rollback ->25. # PRIOR 2026-06-03 (Chris): 1->25, matching HIGH. The 1c gate was a TAKER-crossing finding ("crossing >1c pays away the edge"); but it fired BEFORE the maker-at-mid post, so it blocked ~all real LOW books (6/3: 146 of 388 blend in-window LOW buys killed; real spreads 2-48c, median 19c) and the bot bought ZERO LOW. That defeats the maker-first design: LOW now posts at MID (does NOT pay the spread) + taker-fallback crosses only if the LIVE edge still clears (+ adverse-cancel). So wide spreads are handled here, not gated out. WATCH first fills; revert ->1 if wide-spread LOW bleeds. Prior 6/2 sim (CROSSING): 1c +$154/n16, 2c -$52, 3c+ neg. 0 = off.
...
PUSH_HIGH_NO_MIN_CLEARANCE_F: float = 1.0  # 2026-06-11 (Chris: "deepdive why we can't keep having these losses"): NEW -- a blows-past B-NO must clear the bracket top (cap+0.5) by this many F. The 6/11 DEN-B76.5 loss (mu 77.8, clearance +0.30F, -$9.27) is a STRUCTURAL band, not bad luck: stream clearance bands (38k rows, current gates) -- [0,0.5)F = -7.8c/ct WR33, [0.5,1.0) split-UNSTABLE (odd +19.8/even -10.2), >=1.0F = +15.1c/ct WR83 POSITIVE ALL 4 SPLITS (n=18); combined book w/ both 6/11 gates +17.5..+19.2c/ct vs +14.1. Also catches yesterday's NOLA (clear 0.45). 1.0F ~ 0.85 sigma = "the heat clears the bracket by a full degree." ⚠️What this does NOT fix (named honestly): correlated regional mu-misses -- 6/11 OKC/AUS/DAL (one TX airmass, clearances 1.1-2.6F, -$30 together) pass every entry gate; the only levers there are a daily-entry cap or size. 0=off.
PUSH_HIGH_T_NO_MIN_PNO: float = 0.60  # 2026-06-11: NEW -- T-tail BUY_NO requires model P(NO) >= this (the CHI-T86 6/11 loss: P(NO)=0.53 @28c = cheap-ask-manufactured edge off a coinflip, -$10.23 -- the exact LOW pathology PUSH_LOW_MIN_PNO already gates). Historically FREE: every surviving T-NO in the stream had P(NO)>=0.75, n=0 below 0.60 -> blocks only the pathology. Mirrors the LOW 0.55 floor (0.60 here: T-tails lean on the fragile sigma tail). 0=off.
PUSH_HIGH_NO_SKIP_WONT_REACH: bool = True  # 2026-06-10 (Chris: "deepdive HIGH, most profitable config"): NEW gate -- skip a HIGH B-bracket BUY_NO when mu < floor-0.5 (the "won't-reach" short: betting the day's run-up falls short of the bracket). The one structurally TOXIC cell in the decision-stream replay (38k blend NO rows / 374 tickers / all-markets settled results): wont-reach -32.3c/ct NEGATIVE IN ALL 4 SPLITS (n=10) vs blows-past +10.0 / T-tail +18.8 (both all-splits +); dropping it lifts the kept book +7.8 -> +14.1c/ct (n=43, EVERY split improved; the 8 dropped station-days were 6 losers incl the real 6/9 SEA/ATL). Mechanism: a wont-reach NO has zero obs support -- the running-max ratchet only rises, so one hot hour kills it (the 6/4 + 6/9 + matcher-era "under-projects hot days" family). Combined w/ the thin-margin band 0.5: a HIGH B-NO now requires mu ABOVE cap+0.5 = only short brackets the heat BLOWS PAST. T-tail deep-margin shorts unaffected (+18.8c/ct w/ the empirical tail floor). Same sweep CONFIRMED current knobs: deep window [2.5,4] = the only +EV timing zone (shallow/at-peak -10..-12c/ct), edge 18 split-robust (22 fails odd, >=32 tail -5.8), spread<=5 + price 25-80 hold. REJECTED on discipline: spr<=3 (non-monotone), band 1.5 (contradicts 3148-day recon), P(NO)>=.75 (subsumed by this gate), nwpd<=4 fail-open (+16.9 but the 5/26 n=170 real-fill rejection stands). Rollback ->False.
...
BLEND_DEEP_WINDOW_HOURS: tuple = (3.5, 2.0)  # 2026-06-18 (Chris-approved, blend-era shadow-log window analysis): (5.0,2.5)->(3.5,2.0) => [peak-3.5,peak-2.0]. The 6/17 deepening to peak-5 chased RECONSTRUCTION EV in the phantom-ask zone (6-8h bins reconstruct +EV but have ZERO real fills; [4,5)h reconstructs +2.0c/ct w/ 0 real fills = it added no real volume). Full blend-era shadow-log analysis (197k evals, 5/18-6/18, distinct-ticker, taker@no_ask, current gates, settled): the WINDOW NEAR EDGE is the lever -- pulling it peak-2.5->peak-2.0 (trade 30min CLOSER to peak = sharper forecast + more liquid book) flips H2 (recent half) from NEG to POS across ALL deep-end choices. [peak-3.5,peak-2.0] = the ONLY window positive in all 4 splits: +8.5c/ct (vs current +3.4), H1+11.3/H2+5.0, odd+4.6/even+10.7, WR73 n51, LODO worst-drop +6.6. Excludes the unstable [3.5,4)h bin (H2-9.7/even-9.2 -- where the bot over-concentrated real fills). Mechanism = the OPPOSITE of the phantom deep-window trap (closer not deeper). Judge on settled fills via tools/replay_backtest.py; REVERT ->(4.0,2.0) for +volume/H2~breakeven, or ->(5.0,2.5) to undo the whole thing. # PRIOR (5.0,2.5) 6/17 autonomous deepening [full rationale in git history]
