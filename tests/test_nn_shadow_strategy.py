...
class TestRmTruncation(unittest.TestCase):
    ...
    def test_truncation_shrinks_buy_no_edge_when_rm_in_yes(self):
        # KXHIGHTMIN-B66.5 case. nn raw μ=62.4, σ=2.53. rm=66 (inside YES).
        # Untruncated: P(YES)=0.30 → edge_no=0.68 (the false fat edge).
        # Truncated: nn's mass below rm is impossible; redistributing it
        # to the constraint region pushes most mass into YES (which starts
        # at rm). So P(YES|C) should be much HIGHER than untruncated and
        # the BUY_NO edge much SMALLER.
        d = nss.pure_nn_decide(_pkt(
            floor=66, cap=67,
            ...
            local_clock={"past_peak_today": True, "past_min_today": True},
            # avoid hitting the in-function 25pp ceiling so we can inspect the side
        ), edge_max=1.0)
        # P(YES | day_high ≥ 66) should be substantially higher than the
        # untruncated 0.30 — most remaining mass falls in [66, 67.5).
        self.assertIsNotNone(d["p_yes"])
        self.assertGreater(d["p_yes"], 0.60)
        # Side should still be BUY_NO (P(NO)=0.28 > market's 0.02) but the
        # edge should be much smaller than untruncated 67.8pp.
        self.assertEqual(d["side"], "BUY_NO")
        self.assertLess(d["edge"], 0.35)
        self.assertGreater(d["edge"], 0.20)
